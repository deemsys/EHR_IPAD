//
//  neckmidbackdisabilityViewController.m
...
- (IBAction)nextaction:(id)sender
{
    ...
    if (c==1) {
        [self performSegueWithIdentifier:@"neckback1" sender:self];
...
- (BOOL)shouldPerformSegueWithIdentifier:(NSString *)identifier sender:(id)sender
{
    ...
    if([identifier isEqual:@"neckback1"])
    {
        if (c==1)
        {
            return YES;
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([segue.identifier isEqualToString:@"neckback1"])
    {
        
        neckmidbackdisability1ViewController *destViewController = [segue destinationViewController];
        destViewController.recorddict=recorddict;
        NSLog(@"recorddict in neck back first %@",recorddict);
    }
